...
This is an interface-gated sandbox, not canonical edition integration. The following earlier PM declarations are opaque signature-only stubs: `PM1:✱3·37`, `PM1:✱4·14`, `PM1:✱4·15`, `PM1:✱4·22`. Their bodies are unavailable in this context. A result may cite them only when already in that declaration's exact whitelist; it is blocked from canonical CI/promotion until every stub is remapped one-to-one to an accepted kernel body. Do not attempt to prove, redefine, unfold, or replace an interface.
